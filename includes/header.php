...
if (!defined('BASE_URL')) {
    ...
    $scriptDir = rtrim(dirname($_SERVER['SCRIPT_FILENAME']), '/\\');
    $relative  = str_replace($docRoot, '', $scriptDir);
    $depth     = $relative ? substr_count(trim($relative, '/'), '/') + 1 : 0;
    define('BASE_URL', $depth > 0 ? str_repeat('../', $depth) : '/');
}
if (!defined('PAGE_URL')) {
    define('PAGE_URL', BASE_URL . 'public/');
}
...
<header class="navbar-custom">
    <div class="nav-container-custom">
        <a href="<?php echo PAGE_URL; ?>index.php" class="brand-logo">LC Quiromasajes</a>

        <nav class="nav-center" id="navMenu">
            <ul class="nav-links-list">
                <li><a href="<?php echo PAGE_URL; ?>index.php">Inicio</a></li>
                ...
                <li><a href="<?php echo PAGE_URL; ?>resenas.php">Reseñas</a></li>
            </ul>
        </nav>
        ...
        <div class="nav-right">
            ...
            <?php if (!isset($_SESSION['user_id'])): ?>
                <div class="auth-buttons-header">
                    <a href="<?php echo BASE_URL; ?>auth/login.php" class="login-link-header">Iniciar Sesión</a>
                    <a href="<?php echo BASE_URL; ?>auth/registro.php" class="btn btn-registro-header">Registrarse</a>
                </div>
            <?php else: ?>
                <div class="contenedor-usuario-header">
                    <div id="userMenuBtn" class="btn-usuario">
                        <div class="avatar-circulo"><?php echo strtoupper(substr($_SESSION['nombre_real'] ?? 'U', 0, 1)); ?></div>
                        <span class="nombre-usuario"><?php echo htmlspecialchars($_SESSION['nombre_real'] ?? 'Usuario'); ?></span>
                        <span class="flecha-dropdown">▼</span>
                    </div>
                    <div class="menu-desplegable" id="userDropdown">
                        <div class="menu-cabecera"><p>Mi Cuenta</p></div>
                        <a href="<?php echo PAGE_URL; ?>mis_citas.php">📅 Mis Citas</a>
                        <a href="<?php echo PAGE_URL; ?>mis_pedidos.php">🛍️ Mis Pedidos</a>
                        <a href="<?php echo PAGE_URL; ?>perfil.php">👤 Mi Perfil</a>
